...
class FiltersControllerTest < ActionController::TestCase
  ...
  test "should create filter" do
    assert_difference('Filter.count') do
      post :create, filter: { name: @filter.name, template: @filter.template, text: @filter.text }
    end
  ...
  test "should update filter" do
    patch :update, id: @filter, filter: { name: @filter.name, template: @filter.template, text: @filter.text }
    assert_redirected_to filter_path(assigns(:filter))
  end
